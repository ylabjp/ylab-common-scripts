...
#: 集計が探すファイル名 (セッションのフォルダに 1 つ)。
RESULT_FILENAME = "mepsc_result.json"
#: 人が見るための、イベント 1 つ 1 行の表。
EVENTS_CSV_FILENAME = "mepsc_events.csv"
...
def save_result(session_dir: str | Path, result: MepscResult,
    ...
    """セッションのフォルダに :data:`RESULT_FILENAME` を書く。書いたパスを返す。

    **別名に書いてから差し替える。** 途中で止まったときに壊れた json が最終名で
    残ると、次の集計がそこで止まる (sorter の ``_df.h5`` と同じ理由)。
    """
    ...
    tmp.replace(out)
    if write_csv:
        export_events_csv(out_dir / EVENTS_CSV_FILENAME,
