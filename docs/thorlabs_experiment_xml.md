...
先に外すのには実利もある。プレビューは面数も縦横も本体と違うので、`_page_counts`
が読む面数の集計に混ざると「面数が揃っていない」と判定されてしまう。

### ファイルサイズは面数の代わりにならない

一時期、面数の食い違いを **ファイルサイズ** で見つけようとしていた。生の
ThorLabs TIFF は無圧縮なのでサイズは面数にほぼ比例する、という前提である。

**この前提は実データで破れた。** 3001 枚のうち `ChanA_001_001_001_004.tif`
だけが 6000 面だったのに、サイズは他と変わらなかった (圧縮された別物が置かれて
いたとみられる)。サイズで絞り込む限りこの取りこぼしは無くならないので、
面数は **全ファイルのヘッダを読んで** 確かめる。往復は件数ぶん増えるが、
1 件あたりはヘッダ数 KB の読み取りで、時間のほとんどは往復の待ちである。
待ちは並べれば重なるので `_PROBE_WORKERS` (16) 本で読む。

往復 30 ms を仮定した 6002 件 (3001 枚 x 2ch) の実測: 直列 180 s に対して
並列 11.6 s。取りこぼして 267 秒走ったあとに `Page count mismatch` で落ちるより安い。

### OME-TIFF は「1 データセットが複数ファイルにまたがる」形を持てる

**取り込みで最も驚いた仕様。** OME-TIFF では、先頭ファイルの OME XML が
`<TiffData><UUID FileName="ChanA_..._0002.tif">` のように **兄弟ファイルを名前で指す**
ことができる。tifffile はその名前を追って同じフォルダのファイルを次々に開き、
1 つの配列に組み上げて返す。ファイル自身が持つ画素は 1 面でも、`imread` は
データセット全体を返す。

実データで、取得フォルダの `ChanA_001_001_001_004.tif` にこの XML が入っていた。

```
len(tf.pages) : 1                         ← 自分が持つ面は 1 枚
series[0]     : (3000, 2, 512, 512) TCYX  ← 兄弟を束ねた取得全体
is_ome        : True
宣言サイズ 3,145,728,000 bytes / 実サイズ 1,439,142 bytes
```

実サイズの内訳は「512x512 uint16 が 1 面 = 524,288」+「OME XML 約 893 KB」。
**ファイルが壊れているのではなく、これで正常** である。画素は兄弟ファイルの側にある。

兄弟 6 枚を指す OME マスターを 1 枚読んだときの実測:

| 設定 | 開いた tif | `imread` の shape | 返る画素 |
|---|---|---|---|
| 既定 (multifile) | 6 個 | (6, 8, 8) | 兄弟 6 枚の中身 |
| `_multifile=False` | 1 個 | (8, 8) | 自分の 1 面だけ |

この取り込みは面の配置をファイル名と `Experiment.xml` で決めているので、tifffile に
束ねてもらう必要が無い。束ねられると

1. 1 枚読むのに数千往復かかる (実際「読み込みが非常に長い」として観測された)
2. 宣言された面数と自分の面数が食い違い `Page count mismatch` になる

ので、**生ファイルを開くときは必ず `_multifile=False`** を付ける
(`_ONE_FILE_ONLY`)。1 ファイルは 1 ファイルとして読む。

### 面数は `len(tf.pages)` ではなく `tf.series[0]` で数える

読み取り側 (`_read_file_planes`) が使う `tifffile.imread` が返すのは **series[0]** で、
`len(tf.pages)` とは違う数になりうる。

| ファイル | `len(tf.pages)` | `imread` が返す面数 |
|---|---|---|
| 通常の 1 面 | 1 | 1 |
| 多ページ 6 面 | 6 | 6 |
| 縮小サムネイル付き | **2** | **1** |
| IFD 1 つ + 記述子で N 面を宣言 (ImageJ の連続ハイパースタック等) | **1** | **N** |

検査側と読み取り側で数える対象が違うと、**全件のヘッダを読んでいても検査が素通りする**。
最後の行がそれで、ヘッダは「1 面」と答えるのにグラフを実行すると N 面が返り、
compute の時点で `Page count mismatch` になる。両方を `tf.series[0]` に揃えれば
この食い違いは原理的に起きない。
