            ...
            ExperimentPeriod(
                ...
                mice=[
                    PlanMouse(prj="prj27-3-5", mouse_id="m1", sex="m",
                              ...
                              water_adjust={"day01": 1.7},
                              task_param={"day02": "special.json"},
                              photometry_param={"day02": "mouse_405_override.json"},
                              cond="DEM-Cumin",
...
def test_round_trip():
    ...
    assert p0.mice[0].water_adjust["day01"] == 1.7
...
def test_per_mouse_photometry_round_trip():
    with tempfile.TemporaryDirectory() as d:
        ...
        loaded = load_plan(p)
    assert loaded.periods[0].mice[0].photometry_param["day02"] == "mouse_405_override.json"
...
def test_find_scheduled_mice_resolves_overrides():
    ...
    assert by_id["m1"].day_label == "day02"
    ...
    # day03 photometry override (day-level) reaches both mice
...
def test_find_scheduled_mice_window0_and_sort():
    ...
    assert [s.day_label for s in only_today] == ["day01", "day01"]   # day01 only
